...
const PaymentConfirmation: React.FC<PaymentConfirmationProps> = ({ 
  ...
  const paymentId = paymentData?.paymentId || `PAY-${Math.random().toString(36).substr(2, 12).toUpperCase()}`;
  const orderId = paymentData?.orderId || `ORD-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    ...
    <div className="min-h-screen bg-[var(--smoky-black-1)] relative overflow-hidden flex items-center justify-center p-4 md:p-6">
          ...
          {[...Array(30)].map((_, i) => (
            ...
            >
              <div 
                ...
                style={{
                  backgroundColor: Math.random() > 0.5 ? 'var(--gold-crayola)' : 'var(--white)'
                }}
              ></div>
            </div>
          ))}
      ...
      <div className="w-full max-w-sm mx-auto relative z-30">
        ...
        <div className="text-center">
          ...
          <div className={`relative mb-6 md:mb-8 transform transition-all duration-1000 ${
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-28 h-28 md:w-32 md:h-32 bg-[var(--gold-crayola)] opacity-10 rounded-full animate-ping" style={{ animationDelay: '0.5s' }}></div>
            </div>
          ...
          {/* Success Message */}
          <div className={`mb-6 md:mb-8 transform transition-all duration-800 delay-300 ${
            ...
            <div className="bg-[var(--eerie-black-2)] rounded-xl md:rounded-2xl p-4 md:p-6 border border-[var(--eerie-black-3)] shadow-lg">
              <div className="flex justify-between items-center mb-3 md:mb-4">
                <span className="text-[var(--quick-silver)] text-xs md:text-sm">Amount Paid</span>
                <span className="text-[var(--gold-crayola)] text-xl md:text-2xl font-bold">₹{amount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center mb-3 md:mb-4">
                <span className="text-[var(--quick-silver)] text-xs md:text-sm">Order ID</span>
                <span className="text-[var(--white)] text-xs md:text-sm font-mono truncate max-w-[120px] md:max-w-[160px]">{orderId}</span>
              </div>
              <div className="flex justify-between items-center mb-3 md:mb-4">
                <span className="text-[var(--quick-silver)] text-xs md:text-sm">Payment ID</span>
                <span className="text-[var(--white)] text-2xs md:text-xs font-mono truncate max-w-[120px] md:max-w-[160px]">{paymentId}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[var(--quick-silver)] text-xs md:text-sm">Status</span>
          ...
          <div className={`space-y-2 md:space-y-3 transform transition-all duration-700 ${
