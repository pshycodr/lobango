...
export default function OrderStatusTracker({ currentStatus }: OrderStatusTrackerProps) {
    ...
    const getStatusSteps = (status: string): StatusStep[] => {
        ...
        const steps = [
            ...
            },
            {
                ...
                isCompleted: ['accepted', 'out for delivery', 'delivered'].includes(normalizedStatus),
    ...
    useEffect(() => {
        ...
        const lineTimeout = setTimeout(() => {
            ...
            if (completedCount > 1) {
                ...
                const targetProgress = ((completedCount - 1) / (totalSteps - 1)) * 100;
                setLineProgress(targetProgress);
            }
        }, steps.length * 200 + 300); // Wait for all steps to appear + small delay
    ...
    const getStatusMessage = (status: string) => {
        switch (status.toLowerCase()) {
            case 'pending':
                return { message: "Order is being processed", color: "text-amber-400", bgColor: "bg-amber-400/10", borderColor: "border-amber-400/30" };
            case 'accepted':
                return { message: "Kitchen is preparing your order", color: "text-blue-400", bgColor: "bg-blue-400/10", borderColor: "border-blue-400/30" };
            case 'out for delivery':
                return { message: "Order is out for delivery", color: "text-[var(--gold-crayola)]", bgColor: "bg-[var(--gold-crayola)]/10", borderColor: "border-[var(--gold-crayola)]/30" };
            case 'delivered':
                return { message: "Order delivered successfully", color: "text-green-400", bgColor: "bg-green-400/10", borderColor: "border-green-400/30" };
            case 'rejected':
                return { message: "Order could not be processed", color: "text-red-400", bgColor: "bg-red-400/10", borderColor: "border-red-400/30" };
            default:
                return { message: "Tracking order status", color: "text-[var(--quick-silver)]", bgColor: "bg-[var(--quick-silver)]/10", borderColor: "border-[var(--quick-silver)]/30" };
        }
    };
    ...
    const renderIcon = (step: StatusStep) => {
        ...
        return <div className="w-3 h-3 border-2 border-[var(--quick-silver)]/50 rounded-full bg-[var(--smoky-black-3)]"></div>;
    };
    ...
    return (
        <div className="bg-[var(--eerie-black-2)] rounded-xl p-6 md:p-8 border border-[var(--white-alpha-10)] shadow-lg">
            <div className="text-center mb-8">
                <h2 className={`${playfair.className} text-2xl md:text-3xl font-bold text-[var(--gold-crayola)] mb-4`}>
                    Order Tracking
                </h2>
                <div className={`${statusInfo.bgColor} ${statusInfo.color} ${statusInfo.borderColor} border px-6 py-3 rounded-full inline-flex items-center space-x-3`}>
                    <div className={`w-2 h-2 ${statusInfo.color.replace('text-', 'bg-')} rounded-full ${!isRejected ? 'animate-pulse' : ''}`}></div>
            ...
            <div className="relative max-w-lg mx-auto">
                ...
                {!isRejected && animatedSteps.length > 1 && (
                    <div className="absolute left-4 top-12 w-0.5 overflow-hidden" style={{ height: `${(animatedSteps.length - 1) * 80}px` }}>
                        {/* Background line */}
                ...
                <div className="space-y-8">
                    {animatedSteps.map((step, index) => (
                        <div
                            key={step.id}
                            className={`relative flex items-start space-x-4 transform transition-all duration-500 ease-out ${index < animatedSteps.length ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'
                                }`}
            ...
            {currentStatus.toLowerCase() === 'delivered' && (
                <div className="mt-8 bg-green-500/10 rounded-xl p-6 border border-green-500/20">
                    <div className="text-center">
                        ...
                        <h3 className="text-green-400 font-bold text-lg mb-2">Order Completed</h3>
                        <p className="text-[var(--quick-silver)] text-sm">Thank you for your business!</p>
                    </div>
                </div>
            )}
