...
interface StatusChangeModalProps {
    ...
    onStatusChange: (bookingId: string, newStatus: 'pending' | 'accepted' | 'rejected') => void;
}
        ...
        <div className="fixed inset-0 bg-[var(--black-alpha-80)] flex items-center justify-center z-50 p-4">
            <div className="bg-[var(--eerie-black-1)] border border-[var(--eerie-black-4)] rounded-xl p-6 w-full max-w-md mx-auto">
                ...
                <div className="mb-6 p-4 bg-[var(--eerie-black-2)] rounded-lg">
                    <div className="text-[var(--white)] font-medium mb-2">#{booking.booking_id}</div>
                    <div className="text-[var(--quick-silver)] text-sm mb-1">{booking.customer_name}</div>
                            ...
                            {statusOptions.map((option) => (
                                ...
                                >
                                    <input
                                        ...
                                        onChange={(e) => setSelectedStatus(e.target.value as 'pending' | 'accepted' | 'rejected')}
                                        className="sr-only"
                                        disabled={isLoading}
                                    />
                                    <div className={`w-4 h-4 rounded-full border-2 mr-3 flex items-center justify-center ${selectedStatus === option.value
                                            ? 'border-[var(--gold-crayola)] bg-[var(--gold-crayola)]'
                                            : 'border-[var(--quick-silver)]'
                                        }`}>
                                        {selectedStatus === option.value && (
                                            <Check size={10} className="text-[var(--smoky-black-1)]" />
                                        )}
                                    </div>
                                    <span className="text-[var(--white)] capitalize">{option.label}</span>
                                </label>
                            ))}
                        </div>
                    </div>
