...
export function FloatingCart() {
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 lg:slide-in-from-bottom-8 lg:slide-in-from-right-8 fixed right-0 bottom-0 left-0 z-50 flex items-center justify-between transition-all duration-500 lg:right-6 lg:bottom-6 lg:left-auto lg:w-auto lg:rounded-xl">
      <button
        onClick={handleGoToCart}
        className="group hover:shadow-3xl flex w-full items-center justify-between gap-3 rounded-lg border border-(--white-alpha-20) bg-(--eerie-black-1) p-4 shadow-2xl backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:bg-(--white-alpha-10) lg:w-auto lg:justify-start lg:gap-4"
      >
        {/* Cart Icon with Badge */}
        <div className="relative animate-pulse">
          <div className="border-opacity-30 bg-opacity-20 group-hover:bg-opacity-30 flex h-10 w-10 items-center justify-center rounded-full border border-(--gold-crayola) bg-(--gold-crayola) transition-all duration-300 group-hover:scale-110">
            <ShoppingCart
